...
pfnExitHandler g_ExitHandler[VMX_MAX_GUEST_VMEXIT] =
{
	...
	VmExitEptViolation, // 48 EPT违规
	VmExitEptMisconfig, // 49 EPT配置错误
...
inline BOOLEAN VmxAdvanceGuestRip(IN PGUEST_STATE GuestState)
{
	...
	return VmxSafeVmwrite(GUEST_RIP, GuestState->GuestRip);
}
...
)
{
	...
	__try {
		...
		// 恢复段寄存器（如果需要）
		VmxVMCleanup(pGuestState->GpRegs->SegDs, pGuestState->GpRegs->SegFs);
...
)
{
	...
	__try {
		...
		// 从VMCS读取客户机状态
		pGuestState->GuestEFlags.All = VmcsRead(GUEST_RFLAGS);
		pGuestState->GuestRip = VmcsRead(GUEST_RIP);
		pGuestState->GuestRsp = VmcsRead(GUEST_RSP);
		pGuestState->ExitReason = VmcsRead(VM_EXIT_REASON) & 0xFFFF;
		pGuestState->ExitQualification = VmcsRead(EXIT_QUALIFICATION);
		pGuestState->LinearAddress = VmcsRead(GUEST_LINEAR_ADDRESS);
		pGuestState->PhysicalAddress.QuadPart = VmcsRead(GUEST_PHYSICAL_ADDRESS);

		// 验证退出原因的合理性
		if (pGuestState->ExitReason >= VMX_MAX_GUEST_VMEXIT) {
			DPRINT("HyperHook: CPU %d: 无效的退出原因: %d\n",
				   CPU_INDEX, pGuestState->ExitReason);
			return FALSE;
		}
...
DECLSPEC_NORETURN EXTERN_C VOID VmxExitHandler(IN PCONTEXT Context)
{
	...
	__try {
		...
		PIVCPU Vcpu = &g_HvData->Intel.VmxCpuData[CPU_INDEX];
...
VOID VmExitCR(IN PGUEST_STATE pGuestState)
{
	...
	if (crQual->Fields.Register > 15) {
		DPRINT("HyperHook: CPU %d: 无效的寄存器编号 %d\n",
			   CPU_INDEX, crQual->Fields.Register);
		...
		case TYPE_MOV_TO_CR:
			{
				...
				switch (crQual->Fields.ControlRegister)
				{
					...
					case 4: // CR4
						...
						break;
					default:
						DPRINT("HyperHook: CPU %d: %s: 不支持的控制寄存器编号 %d\n", CPU_INDEX, __FUNCTION__, crQual->Fields.ControlRegister);
						ASSERT(FALSE);
						break;
				}
			}
		...
		case TYPE_MOV_FROM_CR:
			{
				...
				{
					case 0: // CR0
						__vmx_vmread(GUEST_CR0, pRegValue);
						break;
					case 3: // CR3
						__vmx_vmread(GUEST_CR3, pRegValue);
						break;
					case 4: // CR4
						__vmx_vmread(GUEST_CR4, pRegValue);
						break;
					default:
						DPRINT("HyperHook: CPU %d: %s: 不支持的控制寄存器编号 %d\n", CPU_INDEX, __FUNCTION__, crQual->Fields.ControlRegister);
						ASSERT(FALSE);
				...
				{
					__vmx_vmwrite(GUEST_RSP, *pRegValue); // 同步更新 VMCS 的 RSP
				}
			}
			break;

		default:
			DPRINT("HyperHook: CPU %d: %s: 不支持的操作类型 %d\n", CPU_INDEX, __FUNCTION__, crQual->Fields.AccessType);
			ASSERT(FALSE);
			break;
	}
...
VOID VmxHandleNmi(IN PGUEST_STATE pGuestState, IN PINTERRUPT_INFO_FIELD pEvent)
{
	...
	DPRINT("HyperHook: CPU %d: NMI handled at RIP 0x%p\n",
		   CPU_INDEX, pGuestState->GuestRip);
}
	...
	switch (pEvent->Fields.Vector)
	{
		...
		case VECTOR_INVALID_TSS_EXCEPTION:
			DPRINT("HyperHook: CPU %d: 无效TSS异常 at RIP 0x%p, ErrorCode: 0x%llX\n",
				   CPU_INDEX, pGuestState->GuestRip, ErrorCode);
			break;
		...
		case VECTOR_PAGE_FAULT_EXCEPTION:
			DPRINT("HyperHook: CPU %d: 页错误异常 at RIP 0x%p, ErrorCode: 0x%llX, LinearAddr: 0x%p\n",
				   CPU_INDEX, pGuestState->GuestRip, ErrorCode, pGuestState->LinearAddress);
			break;
		...
		default:
			DPRINT("HyperHook: CPU %d: 未知硬件异常 (vector = 0x%X) at RIP 0x%p\n",
				   CPU_INDEX, pEvent->Fields.Vector, pGuestState->GuestRip);
			break;
	}

	// 如果有错误码，设置到VMCS
	if (bHasErrorCode) {
		VmxSafeVmwrite(VM_ENTRY_EXCEPTION_ERROR_CODE, ErrorCode);
	}
