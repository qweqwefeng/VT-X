...
NTSTATUS PHHook( IN PVOID pFunc, IN PVOID pHook )
{
    ...
    // No CPU support
    if (!g_HvData->HvFeatures.VmxFeatures.EPT || !g_HvData->HvFeatures.VmxFeatures.ExecOnlyEPT)
    ...
    PPAGE_HOOK_ENTRY pHookEntry = ExAllocatePoolWithTag( NonPagedPool, sizeof( PAGE_HOOK_ENTRY ), HV_POOL_TAG );
    if (pHookEntry == NULL)
        return STATUS_INSUFFICIENT_RESOURCES;
    ...
    {
        ExFreePoolWithTag( pHookEntry, HV_POOL_TAG );
        return status;
    }
...
{
    // No CPU support
    if (!g_HvData->HvFeatures.VmxFeatures.ExecOnlyEPT)
        return STATUS_NOT_SUPPORTED;
