...
NTSTATUS SHInitHook()
{
	...
	if (!UtilSSDTBase())
	{
		DPRINT("HyperHook: CPU %d: %s: SSDT base not found\n", CPU_INDEX, __FUNCTION__);
		return STATUS_NOT_FOUND;
	}

	// KiSystemServiceCopyEnd
	// F7 05 86 09 3A 00 01 00 00 00 
	// 0F 85 93 04 00 00
	// F7 05 FE 07 3A 00 40 00 00 00
	if (KiServiceCopyEndPtr == 0)
	{
		CHAR pattern[] = "\xF7\x05\xcc\xcc\xcc\xcc\xcc\xcc\xcc\xcc\x0F\x85\xcc\xcc\xcc\xcc\xF7\x05\xFE";
		status = UtilScanSection(".text", (PCUCHAR)pattern, 0xCC, sizeof(pattern) - 1, (PVOID)&KiServiceCopyEndPtr);
		if (!NT_SUCCESS(status))
		{
			DPRINT("HyperHook: CPU %d: %s: KiSystemServiceCopyEnd not found\n", CPU_INDEX, __FUNCTION__);
			return status;
		}
	}
