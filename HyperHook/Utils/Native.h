...
typedef struct _KSPECIAL_REGISTERS
{
    ...
    KDESCRIPTOR Gdtr;              // GDTR
    KDESCRIPTOR Idtr;              // IDTR
    USHORT      Tr;                // 任务寄存器
    USHORT      Ldtr;              // 局部描述符表寄存器
    ULONG       MxCsr;             // MXCSR寄存器
    ULONG64     DebugControl;      // 调试控制
    ...
    ULONG64     Cr8;               // 控制寄存器8
    ULONG64     MsrGsBase;         // GS基址MSR
    ULONG64     MsrGsSwap;         // GS交换MSR
    ULONG64     MsrStar;           // STAR MSR
    ULONG64     MsrLStar;          // LSTAR MSR
    ULONG64     MsrCStar;          // CSTAR MSR
    ULONG64     MsrSyscallMask;    // SYSCALL掩码MSR
