...
inline VOID VmxInjectEvent(INTERRUPT_TYPE InterruptType, VECTOR_EXCEPTION Vector, ULONG WriteLength)
{
	...
	__vmx_vmwrite(VMCS_CTRL_VMENTRY_INTERRUPTION_INFO_FIELD, InjectEvent.All); // 写入 VMCS 注入中断信息字段
	if (WriteLength > 0)
		__vmx_vmwrite(VMCS_CTRL_VMENTRY_INSTRUCTION_LENGTH, WriteLength); // 写入指令长度（用于跳过指令）
}